...
export type AddRawIngredientDialogProps = {
  open: boolean;
...
export function AddRawIngredientDialog({
  open,
  onOpenChange,
  ingredients,
  excludeIds,
      ...
      <DialogContent hideCloseButton className="gap-0 overflow-hidden p-0 sm:max-w-md">
        <div className="relative bg-primary px-10 py-3">
          ...
          <DialogTitle className="text-center text-base font-semibold text-primary-foreground">
            {t("ingredient.recipe.addRawTitle", "Add Raw Ingredient")}
          </DialogTitle>
        </div>
        <div className="space-y-3 p-4">
          <div className="relative">
            <Input
              ...
              placeholder={t("ingredient.recipe.searchIngredients", "Search Ingredients")}
              className="pr-9"
            />
          ...
          {options.length === 0 ? (
            <p className="py-6 text-center text-sm text-muted-foreground">
              {t("ingredient.recipe.noRawMatch", "No raw ingredients available.")}
            </p>
          ) : (
