...
interface ObjectiveHierarchyDialogProps {
  ...
  cycleIds: string[];
  /** Optional plan date to filter cycles (reserved for parity with task dialogs) */
  planDate?: Date | null;
}
...
export const ObjectiveHierarchyDialog: React.FC<ObjectiveHierarchyDialogProps> = ({
  ...
  const { data: allDepartmentObjectivesRaw = [] } = useDepartmentObjectives(organizationId, undefined, false);
  ...
  const companyObjectivesRaw = React.useMemo(() => {
    ...
    return allCompanyObjectivesRaw.filter((co: { cycle_id?: string }) =>
  ...
  const departmentObjectivesRaw = React.useMemo(() => {
    ...
    return allDepartmentObjectivesRaw.filter((dept: { cycle_id?: string }) =>
  ...
  const companyObjectives = React.useMemo(() => {
    ...
    const companyObjectiveIds = new Set(companyObjectivesRaw.map((co: { id: string }) => co.id));
    const deptByCompany = new Map<string, unknown[]>();
    departmentObjectivesRaw.forEach((dept: { company_objective_id?: string }) => {
    ...
    if (deptByCompany.size === 0) {
      allDepartmentObjectivesRaw.forEach((dept: { company_objective_id?: string }) => {
        if (dept.company_objective_id && companyObjectiveIds.has(dept.company_objective_id)) {
          if (!deptByCompany.has(dept.company_objective_id)) deptByCompany.set(dept.company_objective_id, []);
          deptByCompany.get(dept.company_objective_id)!.push(dept);
        }
    ...
    return companyObjectivesRaw.map((co: { id: string }) => ({
      ...co,
  ...
  const { data: individualObjectivesFiltered = [], isLoading: isLoadingIndividualFiltered } =
    useIndividualObjectives(organizationId, cycleIds);
  ...
  const departmentObjectiveIds = React.useMemo(() => {
    ...
    companyObjectives.forEach((co: { department_objectives?: unknown[]; key_results?: unknown[] }) => {
      (co.department_objectives || co.key_results || []).forEach((dept: { id: string }) => ids.add(dept.id));
    });
    return ids;
  }, [companyObjectives]);
  ...
  const individualObjectives = React.useMemo(() => {
    ...
    if (departmentObjectiveIds.size > 0 && individualObjectivesFiltered.length === 0) {
      individualObjectivesAll.forEach((indiv) => {
  ...
  const standaloneObjectives = useMemo(() => {
    return individualObjectives.filter((indiv) => !indiv.department_objective_id);
      ...
      .map((company) => {
        ...
        const departments = company.departments
          .map((dept) => {
            ...
            const individuals = dept.individuals.filter(
              (indiv) =>
  ...
  const filteredStandalone = useMemo(() => {
    ...
    return standaloneObjectives.filter(
      (indiv) =>
        indiv.title.toLowerCase().includes(query) || indiv.employees?.full_name.toLowerCase().includes(query),
    );
                      ...
                      <div className="overflow-x-hidden">
                        ...
                        <div className="px-2 md:px-4 pb-1.5 md:pb-2">
                          <div className="flex items-center gap-1.5 md:gap-2">
                            <Progress value={company.progress_percentage || 0} className="flex-1 h-1.5 md:h-2 min-w-0" />
                              ...
                              {company.departments.map((dept) => (
                                <AccordionItem key={dept.id} value={dept.id} className="border-b border-gray-100 overflow-x-hidden">
                                  <div className="overflow-x-hidden">
                                    ...
                                    <div className="px-2 md:px-4 pb-1.5 md:pb-2">
                                      <div className="flex items-center gap-1.5 md:gap-2">
                                        <Progress value={dept.progress_percentage || 0} className="flex-1 h-1.5 md:h-2 min-w-0" />
                                        <span className="text-[10px] md:text-xs font-medium text-gray-600 w-10 md:w-12 text-right flex-shrink-0">
                                        ...
                                        dept.individuals.map((indiv) => (
                                          ...
                                          >
                                            <div className="space-y-1.5 md:space-y-2">
                                              ...
                                              <div className="flex items-center gap-1.5 md:gap-2">
                                                <Progress value={indiv.progress_percentage || 0} className="flex-1 h-1 md:h-1.5 min-w-0" />
                    ...
                    {filteredStandalone.map((indiv) => (
                      <button
                        ...
                        className={cn(
                          "w-full text-left p-1.5 md:p-2 rounded-md border transition-colors",
                          "hover:bg-blue-50 hover:border-blue-300",
                          selectedId === indiv.id ? "bg-blue-100 border-blue-600 border-2" : "bg-white border-gray-200",
                        )}
                        style={{ maxWidth: "calc(100% - 0.25rem)" }}
                      >
                        <div className="space-y-1.5 md:space-y-2">
                          ...
                          {indiv.employees?.full_name && (
                            <div className="text-[10px] md:text-xs text-gray-500">👤 {indiv.employees.full_name}</div>
                          )}
                          ...
                          <div className="text-[10px] md:text-xs text-gray-400 italic hidden md:block">(No Department)</div>
                        </div>
                      </button>
                    ))}
                  </div>
