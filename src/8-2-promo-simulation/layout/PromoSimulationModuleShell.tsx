...
import { PricingToolsHeaderAndTab } from "@/8-2-pricing-tools/components/PricingToolsHeaderAndTab";
import { PromoSimulationPageSkeleton } from "../skeletons/PromoSimulationPageSkeleton";

type PromoSimulationModuleShellProps = {
  children: ReactNode;
  showContent: boolean;
};
...
export function PromoSimulationModuleShell({
  children,
  showContent,
}: PromoSimulationModuleShellProps) {
  return (
    <div className="relative flex h-full min-h-0 w-full min-w-0 flex-1 flex-col overflow-hidden bg-background font-sans">
      ...
      {!showContent ? (
        <div
          className="absolute inset-0 z-20 flex min-h-0 min-w-0 flex-col overflow-auto bg-background"
          aria-busy
        >
