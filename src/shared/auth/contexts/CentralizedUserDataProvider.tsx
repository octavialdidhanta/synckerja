...
import { resetIdentityQueriesForAuthUser, clearOrganizationScopedQueries, seedIdentityQueriesFromCentralSnapshot } from '@/shared/auth/identityQuerySync';
import { fetchUserOrganizations, type UserOrganizationsData, userOrganizationsQueryKey } from '@/shared/hooks/userOrganizationsQuery';
...
import {
  ...
  type CentralizedUserData,
  type CentralizedOrganization,
  type CentralizedEmployee,
  type CentralizedUserRole,
  type CentralizedUserDataContextType,
} from '@/shared/auth/contexts/centralizedUserDataContext.shared';

type UserData = CentralizedUserData;
type Organization = CentralizedOrganization;
type Employee = CentralizedEmployee;
type UserRole = CentralizedUserRole;

// Provider component — hooks live in useCentralizedUserData.ts (Fast Refresh compatible).
export const CentralizedUserDataProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const refreshUserData = useCallback(async () => {
    ...
    let seededUserOrganizations: UserOrganizationsData | null = null;
    ...
    if (!forceRefresh && !emailVerifiedFlag && userDataCacheRef.current) {
      ...
      if (cacheAge < CACHE_DURATION && userDataCacheRef.current.data?.user_id === user.id) {
        ...
        if (user) {
          try {
            if (!queryClient.getQueryData(userOrganizationsQueryKey)) {
              seededUserOrganizations = await fetchUserOrganizations({
                userId: user.id,
                profileActiveOrgId: userDataCacheRef.current.data?.active_organization_id ?? null,
                skipProfileSync: true,
              });
            }
          } catch {
            /* non-critical — header can refetch memberships */
          }
          seedIdentityQueriesFromCentralSnapshot(queryClient, {
            user,
            userData: userDataCacheRef.current.data,
            organization: userDataCacheRef.current.organization,
            userRole: userDataCacheRef.current.userRole,
    ...
    if (lastUserIdRef.current === user.id && !forceRefresh && !emailVerifiedFlag) {
      ...
      if (!cacheIncomplete) {
        ...
        setLoading(false);
        setCentralProfileHydrated(true);
        if (user && userDataCacheRef.current) {
          seedIdentityQueriesFromCentralSnapshot(queryClient, {
            user,
            userData: userDataCacheRef.current.data,
            organization: userDataCacheRef.current.organization,
...
// Set user data
      ...
      if (organizationId) {
        ...
        const employeePromise = (async () => {
          const result = await supabase
            .from('employees')
            .select(
              'id, employee_id, full_name, email, organization_id, department_id, employee_status_id, user_id, job_level_id, profile_photo_url',
            )
            .eq('user_id', user.id)
            .eq('organization_id', organizationId)
            .maybeSingle();
          ...
          const [statusRes, deptRes, levelRes, detailsRes] = await Promise.all([
          ...
          const profilePhotoUrl =
            ...
            detailsRes.data?.profile_photo_url ||
            null;
      ...
      if (userRef.current && !rolesStillPending) {
        const snapForSeed = userDataCacheRef.current;
        seedIdentityQueriesFromCentralSnapshot(queryClient, {
          user: userRef.current,
          userData: snapForSeed?.data ?? userDataStateRef.current,
          organization: snapForSeed?.organization ?? organizationStateRef.current,
          userRole: roleForHydration,
          userOrganizations: seededUserOrganizations,
        });
      }
    }
