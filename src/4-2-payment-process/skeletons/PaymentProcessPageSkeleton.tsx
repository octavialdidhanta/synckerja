...
import { PaymentProcessPageSkeletonFrame } from '@/4-2-payment-process/layout/PaymentProcessPageSkeletonFrame';

/** Skeleton khusus `/expenses/payment-process` — selaras `PaymentProcessPage`. */
export function PaymentProcessPageSkeleton() {
  const { t } = useAppTranslation();
  const aria = t('expenses.paymentProcess.loadingAria', 'Loading payment process');

  return (
    <PaymentProcessPageSkeletonFrame
      ariaLabel={aria}
      toolbar={
        <>
