...
export function GoogleAdsSettingsShell() {
  ...
  useEffect(() => {
    ...
    if (!canManage) {
      toast.message(t("omnichannel.settings.googleAds.adminOnlyToast"));
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      <div className={`${OMNICHANNEL_SETTINGS_CARD_HEADER_BASE} shrink-0`}>
        <h2 className={OMNICHANNEL_SETTINGS_CARD_TITLE_CLASS}>
          {t("omnichannel.settings.googleAds.pageTitle")}
        </h2>
        <p className={OMNICHANNEL_SETTINGS_CARD_SUBTITLE_CLASS}>
          {t("omnichannel.settings.googleAds.intro")}
        </p>
      </div>
