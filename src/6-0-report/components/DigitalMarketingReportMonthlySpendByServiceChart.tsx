...
function formatSpendBarLabel(value: unknown, currency: string | null): string {
  const n = typeof value === "number" ? value : Number(value);
  if (!Number.isFinite(n) || n <= 0) return "";
  return formatMetaMetricValue("spend", n, currency);
}

function resolveSpendLabelValue(
  raw: number | string | Array<number | string> | undefined,
  payload: ReportSpendByServiceChartPoint | undefined,
): number | null {
  ...
  if (payload && Number.isFinite(payload.spend)) return payload.spend;
  return null;
}

function createServiceSpendBarLabelRenderer(currency: string | null) {
  return function ServiceSpendBarLabelContent(props: {
    x?: number | string;
    y?: number | string;
    width?: number | string;
    value?: number | string | Array<number | string>;
    payload?: ReportSpendByServiceChartPoint;
  }) {
    const x = Number(props.x);
    const y = Number(props.y);
    const width = Number(props.width);
    const n = resolveSpendLabelValue(props.value, props.payload);
    if (!Number.isFinite(x) || !Number.isFinite(y) || !Number.isFinite(width) || n == null || n <= 0) {
      return null;
    }
    const text = formatSpendBarLabel(n, currency);
    if (!text) return null;
    return (
      <text
        x={x + width / 2}
...
type ServiceSpendTooltipProps = TooltipProps<number, string> & {
  currency: string | null;
};

function ServiceSpendTooltip({ active, payload, currency }: ServiceSpendTooltipProps) {
  if (!active || !payload?.length) return null;
  const row = payload[0]?.payload as ReportSpendByServiceChartPoint | undefined;
  ...
  return (
    <div className="rounded-md border border-gray-200 bg-white px-3 py-2 text-xs shadow-sm">
      ...
      <p className="mt-0.5 tabular-nums text-gray-900">
        {formatMetaMetricValue("spend", row.spend, currency)}
      </p>
    </div>
  );
}
...
type Props = {
  ...
  chartData: ReportSpendByServiceChartPoint[];
...
export function DigitalMarketingReportMonthlySpendByServiceChart({
  bootstrapLoading,
  channelFilter,
  chartData,
  googleSeries,
  ...
  const hasData = chartData.some((row) => row.spend > 0);
    ...
    <div className={shellClass}>
      {loading ? (
        ...
        <div className="flex h-[300px] items-center justify-center rounded-md bg-gray-50 text-sm text-muted-foreground">
          {t(
            "digitalMarketing.report.monthlySpendByServiceEmptyServices",
            "No services with spend in the selected range.",
          )}
        </div>
      ) : !hasData ? (
        <div className="flex h-[300px] items-center justify-center rounded-md bg-gray-50 text-sm text-muted-foreground">
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <YAxis
                    ...
                    axisLine={false}
                    width={48}
                    tickFormatter={(v) => formatAxisTick(Number(v), currency)}
                  />
                  <Tooltip content={<ServiceSpendTooltip currency={currency} />} />
                  <Bar
                    dataKey="spend"
                    radius={[4, 4, 0, 0]}
                  ...
                  >
                    ...
                    <LabelList
                      position="top"
                      content={createServiceSpendBarLabelRenderer(currency)}
                    />
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
