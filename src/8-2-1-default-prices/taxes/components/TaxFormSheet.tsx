...
import { TaxOutletsSection } from "./TaxOutletsSection";

export type TaxFormSheetProps = {
  tax: CatalogTax | null;
...
export function TaxFormSheet({ tax, selectedOutletId, open, onOpenChange }: TaxFormSheetProps) {
  const { t } = useAppTranslation();
  const { toast } = useToast();
  const { save, isSaving } = useCatalogTaxes();
  ...
  useEffect(() => {
    ...
    if (tax) {
      setName(tax.name);
      setAmountDisplay(formatAmountDisplay(tax.amount_percent));
      setOutletIds([...(tax.outlet_ids ?? [])]);
    } else {
      setName("");
      setAmountDisplay("");
      setOutletIds(selectedOutletId ? [selectedOutletId] : []);
    }
  }, [open, tax, selectedOutletId]);
  ...
  const title = tax
    ? t("defaultPrices.taxes.editTitle", "Edit tax")
    : t("defaultPrices.taxes.addTitle", "New tax");

  const handleSave = async () => {
    if (!nameValid) {
      toast({
        title: t("defaultPrices.taxes.nameRequired", "Enter a tax name."),
        variant: "destructive",
      });
      return;
    }
    if (!amountValid || parsedAmount == null) {
      toast({
        title: t("defaultPrices.taxes.amountInvalid", "Enter an amount between 0 and 100."),
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({ title: t("defaultPrices.taxes.saved", "Tax saved.") });
      onOpenChange(false);
    } catch {
      toast({
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 py-4">
          <section className="space-y-3">
            <p className="border-b pb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">
              {t("defaultPrices.taxes.section", "Tax information")}
            </p>
            <div className="grid grid-cols-[1fr_140px] gap-2">
              <div className="relative">
                <Input
                  ...
                  placeholder={t("defaultPrices.taxes.namePlaceholder", "Name")}
              ...
              <div className="flex">
                <div className="relative min-w-0 flex-1">
                  <Input
                    ...
                    placeholder={t("defaultPrices.taxes.amountPlaceholder", "Amount")}
          ...
          <TaxOutletsSection selectedIds={outletIds} onChange={setOutletIds} />
        </div>
