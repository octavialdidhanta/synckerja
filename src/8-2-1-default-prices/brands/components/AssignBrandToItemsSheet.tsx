...
export type AssignBrandToItemsSheetProps = {
  brand: CatalogBrand | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
};

export function AssignBrandToItemsSheet({ brand, open, onOpenChange }: AssignBrandToItemsSheetProps) {
  ...
  useEffect(() => {
    ...
    setSelectedIds(
      new Set(products.filter((row) => row.product_brand_id === brand.id).map((row) => row.id)),
    );
  }, [open, brand, products]);
  ...
  const handleSave = async () => {
    ...
    const changes = products.flatMap((row) => {
      ...
      const wasThis = row.product_brand_id === brand.id;
      if (checked && !wasThis) {
        return [{ id: row.id, payload: { product_brand_id: brand.id } }];
      }
      if (!checked && wasThis) {
        return [{ id: row.id, payload: { product_brand_id: null } }];
      }
      return [];
    });
    ...
    try {
      ...
      toast({ title: t("defaultPrices.brands.assignSaved", "Products updated.") });
      onOpenChange(false);
    } catch {
      toast({
  ...
  const title = t("defaultPrices.brands.assignDialogTitle", "Assign to {{name}}", {
    name: brand?.name ?? "",
  });
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-4">
          {products.length === 0 ? (
            <p className="py-6 text-sm text-muted-foreground">
              {t(
                "defaultPrices.brands.assignEmpty",
        ...
        <SheetFooter className="shrink-0 border-t px-6 py-4">
          ...
          <Button type="button" onClick={() => void handleSave()} disabled={!brand || busy || products.length === 0}>
            {t("common.save", "Save")}
          </Button>
        </SheetFooter>
      </SheetContent>
