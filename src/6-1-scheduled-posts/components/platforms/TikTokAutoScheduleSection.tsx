...
export function TikTokAutoScheduleSection({
  ...
  const isRateLimitQueue =
    ...
    Boolean(tiktokSchedule.error_message?.startsWith('rate_limited:'));
              ...
              >
                {isRateLimitQueue
                  ? t(
                      ...
                      tiktokSchedule.next_retry_at
                        ? 'Waiting in publish queue — next attempt at {{time}} WIB'
                        : 'Waiting in publish queue — retrying shortly',
                      {
                        time: tiktokSchedule.next_retry_at
                          ? formatTimeWibFromUtc(tiktokSchedule.next_retry_at)
                          : '',
                      },
                    )
                  : tiktokSchedule.status === 'pending'
                    ? t(
                        ...
                        tiktokSchedule.next_retry_at
                          ? 'Publish delayed — next retry at {{time}} WIB. {{error}}'
