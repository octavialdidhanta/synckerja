...
import { useIncomeMetrics } from "@/4-1-dashboard/hooks/useIncomeMetrics";

/** Carousel / summary stats for mobile income transaction page (same fields as `useIncomeMetrics` payload). */
export function useIncomeTransactionDashboardStats() {
  ...
  const thisYearRevenue = useMemo(() => Number(metrics?.yearTotal || 0), [metrics?.yearTotal]);
