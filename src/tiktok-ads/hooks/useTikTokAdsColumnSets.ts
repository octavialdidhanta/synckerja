...
export type TikTokAdsColumnSet = {
  id: string;
  name: string;
  metric_keys: string[];
  scope: "global" | "org";
...
function mapColumnSetRow(row: {
  ...
  created_at?: string;
  updated_at?: string;
} & { scope: TikTokAdsColumnSet["scope"] }): TikTokAdsColumnSet {
  return {
    id: String(row.id),
...
export function useTikTokAdsColumnSets(
  organizationId: string | null | undefined,
  entity: TikTokAdsMetricEntity,
  enabled: boolean,
) {
  const queryClient = useQueryClient();
  const queryKey = ["tiktok-ads-column-sets", "v1", organizationId, entity];
  ...
  const remove = useMutation({
    mutationFn: async (input: { id: string; scope?: TikTokAdsColumnSet["scope"] }) => {
