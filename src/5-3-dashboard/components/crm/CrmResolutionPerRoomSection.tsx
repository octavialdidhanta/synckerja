...
import type { CrmRoomCycleRow } from '@/5-3-dashboard/hooks/useCrmFirstResponsePerRoom';
...
function exportRowsCsv(rows: CrmRoomCycleRow[]) {
  const headers = [
    'Room ID',
    'Customer ID',
    'Agent',
  ...
  const lines = [
    ...
    ...rows.map((r) =>
      [
        ...
        r.sla_resolution_late_minutes != null ? String(r.sla_resolution_late_minutes) : '',
  ...
  a.href = url;
  a.download = `crm-resolution-per-room-${format(new Date(), 'yyyyMMdd-HHmm')}.csv`;
  a.click();
...
/**
 * Same RPC + React Query cache as {@link CrmFirstResponsePerRoomSection}; period filter uses `resolved_at`.
 */
export function CrmResolutionPerRoomSection() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      if (!resolvedAtWithinPeriod(r.resolved_at, period)) return false;
      ...
      const hay = [
        ...
        r.sla_inter_reply_late_minutes != null ? String(r.sla_inter_reply_late_minutes) : '',
        r.resolution_due_at ?? '',
        formatCellTs(r.resolution_due_at),
        r.resolved_at ?? '',
        formatCellTs(r.resolved_at),
      ]
        .join(' ')
        .toLowerCase();
  ...
  const sorted = useMemo(() => {
    ...
    copy.sort((a, b) => {
      ...
      if (col === 'cycle_started_at' || col === 'resolution_due_at' || col === 'resolved_at') {
  ...
  const toggleSort = (col: SortCol) => {
    setSort((s) => {
      ...
      return { col, dir: col === 'resolved_at' ? 'desc' : 'asc' };
    });
  };
  ...
  return (
    <section
      ...
      aria-labelledby="crm-resolution-room-heading"
    >
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          <h2 id="crm-resolution-room-heading" className="text-lg font-semibold text-foreground">
            Resolution time per Room ID
          </h2>
          <span
            className={cn(
        ...
        <Table containerClassName="scrollbar-hide max-h-[min(480px,60vh)] min-h-0">
          <TableHeader>
            <TableRow className="hover:bg-transparent">
              ...
              <TableHead className="whitespace-nowrap">
                <SortBtn col="sla_resolution_status" label={t('crm.sla.colResolution', 'Resolution SLA')} />
              </TableHead>
              <TableHead className="whitespace-nowrap">
                <SortBtn col="sla_inter_reply_status" label={t('crm.sla.colInterReply', 'Inter-reply SLA')} />
              </TableHead>
              <TableHead className="whitespace-nowrap">
                <SortBtn col="cycle_started_at" label={t('crm.sla.colCycleStart', 'Cycle start')} />
              </TableHead>
              <TableHead className="whitespace-nowrap">
                <SortBtn col="resolution_due_at" label={t('crm.sla.colResolutionDue', 'Resolution due')} />
              </TableHead>
              <TableHead className="whitespace-nowrap">
                <SortBtn col="resolved_at" label="Resolution time" />
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {isPending ? (
              ...
              <TableRow>
                <TableCell colSpan={9} className="py-8 text-center text-sm text-muted-foreground">
                  {rows.length === 0 ? (
                    <span className="mx-auto block max-w-xl leading-relaxed">
                      ...
                      tampil. Coba filter{' '}
                      <span className="font-medium">Semua periode</span> atau uji dengan percakapan live chat yang
                      sudah pernah inbound.
                    </span>
                  ) : period !== 'all' ? (
                    'Tidak ada baris: filter periode memakai waktu resolve (bukan tanggal mulai siklus). Percakapan yang belum di-resolve atau resolve di luar jendela tidak tampil — coba All time, periode lebih panjang, atau ubah channel/pencarian.'
                  ) : (
                    'Tidak ada baris untuk filter channel atau pencarian saat ini.'
                  )}
                </TableCell>
              </TableRow>
            ) : (
              pageRows.map((r) => (
                <TableRow key={r.conversation_id}>
                  ...
                  <TableCell className="text-muted-foreground">
                    {formatSlaStatusLabel(t, r.sla_resolution_status, r.sla_resolution_late_minutes)}
                  </TableCell>
                  <TableCell className="text-muted-foreground">
                  ...
                  <TableCell className="whitespace-nowrap text-xs">{formatCellTs(r.resolution_due_at)}</TableCell>
                  <TableCell className="whitespace-nowrap text-xs">{formatCellTs(r.resolved_at)}</TableCell>
                </TableRow>
              ))
            )}
          </TableBody>
