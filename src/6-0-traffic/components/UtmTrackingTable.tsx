...
function UtmColumnSelect({ "aria-label": ariaLabel, value, onValueChange, options }: UtmColumnSelectProps) {
  ...
  return (
    <Select value={value} onValueChange={onValueChange}>
      <SelectTrigger
        className="h-8 w-full min-w-0 border-gray-200 bg-white text-xs [&>span]:truncate"
      ...
      <SelectContent position="popper" className="max-h-64">
        ...
        {hasEmpty ? <SelectItem value={FILTER_EMPTY}>(kosong)</SelectItem> : null}
        {nonEmpty.map((o) => (
          <SelectItem key={o} value={o} title={o} className="text-xs">
            <span className="block max-w-[min(100vw,20rem)] truncate">{o.length > 64 ? `${o.slice(0, 61)}…` : o}</span>
          </SelectItem>
        ))}
      </SelectContent>
    </Select>
...
export function UtmTrackingTable({
  rows,
  ...
  return (
    <div className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
      ...
      <div className="scrollbar-hide seamless-scroll nested-scroll-touch-chain flex-1 min-h-0 overflow-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <table className="min-w-[1550px] w-full table-fixed border-separate border-spacing-0">
