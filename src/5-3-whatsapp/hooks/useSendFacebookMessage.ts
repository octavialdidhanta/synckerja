...
export function useSendFacebookMessage() {
  ...
  const mutation = useMutation({
    mutationFn: async (params: SendFacebookMessageParams) => {
      ...
      const url = `${SUPABASE_URL}/functions/v1/send-facebook-message`;
      ...
      if (!res.ok) {
        ...
        const msg =
          ...
          'Gagal mengirim pesan Messenger.';
        throw new Error(msg);
      }
      return json;
    },
    onSuccess: (data: { success?: boolean; message?: FacebookMessage | null; lead_status_id?: string | null }, variables) => {
      const conversationId = variables.conversation_id;
      if (conversationId && data?.message) {
        queryClient.setQueryData<FacebookMessage[]>(
          ['facebook-messages', conversationId],
          (prev = []) => {
      ...
      if (conversationId) {
        if (!data?.message) {
          queryClient.invalidateQueries({ queryKey: ['facebook-messages', conversationId] });
        }
        const statusQueryKey = ['facebook-conversation-status', conversationId] as const;
        const statusIdFromBackend = data?.lead_status_id ?? null;
        const orgStatuses =
          queryClient.getQueriesData<Array<{ id: string; name: string }>>({ queryKey: ['lead-statuses'] }) ?? [];
        ...
        void queryClient.invalidateQueries({ queryKey: ['facebook-conversations'] });
      }
    },
  });
