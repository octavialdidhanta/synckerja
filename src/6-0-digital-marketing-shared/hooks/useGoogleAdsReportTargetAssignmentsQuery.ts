...
export function useGoogleAdsReportTargetAssignmentsQuery(
  period: GoogleAdsReportTargetPeriodKey | null,
) {
  const { organizationId } = useCurrentOrg();

  return useQuery({
    queryKey: googleAdsReportTargetQueryKeys.assignments(organizationId, period),
    queryFn: async (): Promise<GoogleAdsReportTargetAssignmentRow[]> => {
      ...
      return (data ?? []) as GoogleAdsReportTargetAssignmentRow[];
    },
