...
import {
  aggregateServiceSpendByCalendarMonth,
  buildSpendByServiceTotalsChartPoints,
  buildReportServiceSpendSeriesList,
  serviceIdForMonthlyApi,
  sumMonthlySpendForChannelFilter,
...
function applySpanModeMonths(
  ...
  if (spanMode === "all_time") return aggregateServiceSpendByCalendarMonth(rows);
  return rows;
}

export function useDigitalMarketingReportMonthlySpendByService(args: {
  enabled: boolean;
  ...
  const serviceQueries = useQueries({
    queries: services.map((svc) => ({
      queryKey: [
        "dm-report-monthly-spend-by-service-v1",
        organizationId,
        effectiveGoogleCustomerId,
        effectiveMetaAdAccountId,
        selectedYear,
      ...
      queryFn: async (): Promise<ServiceMonthlyQueryResult> => {
        ...
        if (fetchGoogleEnabled && monthlyChartChannelFilter !== "meta") {
          ...
          currency = payload?.currency_code ?? payload?.currency ?? currency;
        }
        ...
        if (fetchMetaEnabled && monthlyChartChannelFilter !== "google") {
          ...
          const metaCur = normalizeMetaAdsReportCurrency(
            payload?.currency ?? payload?.currency_code,
          );
          currency = currency ?? metaCur;
        }
  ...
  const currency = useMemo(() => {
    for (const q of serviceQueries) {
      const c = q.data?.currency;
      if (c) return c;
    }
    return null;
  }, [serviceQueries]);

  const spendByServiceKey = useMemo(() => {
    const map = new Map<string, Map<string, number>>();
    for (const q of serviceQueries) {
      if (!q.data) continue;
      const spendMap = sumMonthlySpendForChannelFilter(
        q.data.googleMonths,
        q.data.metaMonths,
        selectedYear,
        monthlyChartChannelFilter,
      ...
      map.set(q.data.dataKey, spendMap);
    }
    return map;
  ...
  const chartData = useMemo(
    () =>
      enabled
        ? buildSpendByServiceTotalsChartPoints(services, spendByServiceKey)
        : [],
    [enabled, services, spendByServiceKey],
  );

  return {
    services: services as ReportServiceSpendSeries[],
    chartData,
    loading,
    currency,
    error,
