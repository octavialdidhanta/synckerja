...
export function useDmReportTargetsQuery(period: DmReportTargetPeriodKey | null) {
  const { organizationId } = useCurrentOrg();

  return useQuery({
    queryKey: dmReportTargetQueryKeys.targets(organizationId, period),
    queryFn: async (): Promise<DmReportTargetRow[]> => {
      ...
      return (data ?? []) as DmReportTargetRow[];
    },
