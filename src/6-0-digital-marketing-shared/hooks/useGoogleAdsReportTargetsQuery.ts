...
export function useGoogleAdsReportTargetsQuery(period: GoogleAdsReportTargetPeriodKey | null) {
  const { organizationId } = useCurrentOrg();

  return useQuery({
    queryKey: googleAdsReportTargetQueryKeys.targets(organizationId, period),
    queryFn: async (): Promise<GoogleAdsReportTargetRow[]> => {
      ...
      return (data ?? []) as GoogleAdsReportTargetRow[];
    },
