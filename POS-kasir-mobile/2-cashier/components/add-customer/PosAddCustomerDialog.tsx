...
export function PosAddCustomerDialog({
  ...
  const [phoneViewportH, setPhoneViewportH] = useState(
    () => (typeof window === "undefined" ? 0 : window.innerHeight),
  );

  useEffect(() => {
    if (!open || !isPhone) return;
    const sync = () => setPhoneViewportH(window.innerHeight);
    sync();
  ...
  const header = (titleNode: ReactNode) => (
    <div className="relative z-10 flex shrink-0 items-center justify-center border-b border-slate-100 bg-white px-3 py-3">
      <Button
        type="button"
        variant="outline"
        size="sm"
  ...
  const body = (
    <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden">
      <div
        className="scrollbar-hide seamless-scroll nested-scroll-touch-chain min-h-0 flex-1 overflow-y-auto overflow-x-hidden overscroll-contain px-4 py-4 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        data-vaul-no-drag=""
      >
        <PosMemberLookupPanel
    ...
    return (
      ...
      >
        <DrawerContent
          aboveAppNav={false}
          smoothFast
          className="z-[70] flex h-[min(88dvh,860px)] max-h-full min-h-0 flex-col gap-0 overflow-hidden rounded-t-2xl border-0 p-0 pb-[max(env(safe-area-inset-bottom,0px),0.5rem)] shadow-2xl"
          overlayClassName="z-[70]"
          style={
            phoneViewportH > 0
              ? { maxHeight: phoneViewportH }
              : undefined
          }
        >
          {header(
