...
interface IncomeGrowthCardProps {
  growth: number;
}

export function IncomeGrowthCard({ growth }: IncomeGrowthCardProps) {
      ...
      <CardContent className="p-3 min-w-0 flex flex-col gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-blue-100 truncate">
            {t("incomes.growth", "Growth")}
          </span>
        </div>
        <div className="flex items-center justify-between gap-3 min-w-0">
          <div className="min-w-0 flex-1">
            <div className="text-2xl sm:text-3xl font-bold text-white truncate">{growth.toFixed(1)}%</div>
            <div className="text-xs text-blue-100 truncate mt-1">
              {t("incomes.vsLastMonth", "vs last month")}
            </div>
          </div>
        </div>
      </CardContent>
