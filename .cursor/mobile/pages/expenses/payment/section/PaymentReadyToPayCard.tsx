...
interface PaymentReadyToPayCardProps {
  readyToPay: number;
}

export function PaymentReadyToPayCard({ readyToPay }: PaymentReadyToPayCardProps) {
      ...
      <CardContent className="p-3 min-w-0 flex flex-col gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-blue-100 truncate">
            {t("payment.readyToPay", "Ready to Pay")}
          </span>
        </div>
        <div className="text-2xl sm:text-3xl font-bold text-white truncate">{readyToPay}</div>
      </CardContent>
    </Card>
  );
}
