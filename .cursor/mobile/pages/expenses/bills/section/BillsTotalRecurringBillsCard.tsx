...
interface BillsTotalRecurringBillsCardProps {
  totalRecurringBills: number;
}

export function BillsTotalRecurringBillsCard({ totalRecurringBills }: BillsTotalRecurringBillsCardProps) {
      ...
      <CardContent className="p-3 min-w-0 flex flex-col gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-blue-100 truncate">
            {t("bills.totalRecurringBills", "Total Recurring Bills")}
          </span>
        </div>
        <div className="text-2xl sm:text-3xl font-bold text-white truncate">{totalRecurringBills}</div>
      </CardContent>
    </Card>
  );
}
