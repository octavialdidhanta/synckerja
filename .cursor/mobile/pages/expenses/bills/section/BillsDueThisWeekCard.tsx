...
export function BillsDueThisWeekCard({ dueThisWeek }: BillsDueThisWeekCardProps) {
      ...
      <CardContent className="p-3 min-w-0 flex flex-col gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-blue-100 truncate">
            {t("bills.dueThisWeek", "Due This Week")}
          </span>
        </div>
        <div className="text-2xl sm:text-3xl font-bold text-white truncate">{dueThisWeek}</div>
      </CardContent>
    </Card>
  );
}
