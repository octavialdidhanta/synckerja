...
export function ApprovalsRecurringCard({ recurring }: ApprovalsRecurringCardProps) {
      ...
      <CardContent className="p-3 min-w-0 flex flex-col gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-blue-100 truncate">
            {t("approvals.recurring", "Recurring")}
          </span>
        </div>
        <div className="text-2xl sm:text-3xl font-bold text-white truncate">{recurring}</div>
      </CardContent>
    </Card>
  );
}
