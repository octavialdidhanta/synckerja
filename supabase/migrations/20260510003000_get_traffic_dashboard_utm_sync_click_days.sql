-- UTM Tracking clicks must be synchronized with Source traffic clicks.
-- Keep the existing dashboard RPC as a base function, then replace only `utm_table`
-- with session-day rows built from both page-view sessions and click-event sessions.

ALTER FUNCTION public.get_traffic_dashboard(text, date, date, int, int, int)
  RENAME TO get_traffic_dashboard_base_before_utm_click_sync;
...
BEGIN
  ...
  WITH
    ...
    candidate_days AS (
      ...
      UNION
      SELECT * FROM click_session_days
    ),
    session_base AS (
      SELECT
        cd.web_id,
      ...
      LEFT JOIN public.analytics_sessions s
        ON s.id = cd.session_id
        AND s.web_id = cd.web_id
    ),
    effective AS (
      SELECT
        web_id,
...
COMMENT ON FUNCTION public.get_traffic_dashboard(text, date, date, int, int, int) IS
  'Dashboard RPC wrapper; UTM table uses session-day rows from page views plus click events so clicks stay synchronized with Source traffic.';
