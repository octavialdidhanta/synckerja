-- Click drill-down for hybrid UTM rows: session (all session clicks) vs journey (clicks on page path).
      ...
      WHERE ce.web_id = p_web_id
        ...
        AND (
          ...
          OR (
            v_session_id = ''
            AND v_visitor_id <> ''
            AND eff.visitor_id = v_visitor_id
          )
          OR (
            v_session_id = ''
            AND v_visitor_id = ''
...
COMMENT ON FUNCTION public.get_click_targets_for_utm_row(
  ...
  'Click targets for hybrid UTM row; session = all clicks in session, journey = clicks on page path (first-touch UTM).';
