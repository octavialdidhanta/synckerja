-- Extend apply_catalog_checkout_stock to consume modifier-option recipes
-- when p_lines entries include modifier_option_ids (uuid[] as jsonb array).
...
BEGIN
  ...
  LOOP
    ...
    IF v_tracked IS TRUE THEN
      ...
      -- Base product recipe (no modifier option)
      FOR v_recipe IN
    ...
    -- Modifier-option recipes (independent of product track_stock)
...
COMMENT ON FUNCTION public.apply_catalog_checkout_stock(uuid, uuid, text, jsonb) IS
  'Deduct product + base recipe + selected modifier-option recipe ingredients for store checkout lines.';
