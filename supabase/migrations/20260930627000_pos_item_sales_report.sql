-- Item Sales report (Fase A): per product/variant summary with income + quantity metrics
...
BEGIN
  ...
  sold_grouped AS (
    ...
    GROUP BY
      ...
      l.bundle_id,
      l.cat_id,
      CASE WHEN l.product_id IS NULL THEN lower(btrim(COALESCE(l.service_name, ''))) ELSE NULL END
  ),
  refund_grouped AS (
    SELECT
...
COMMENT ON FUNCTION public.pos_item_sales_report(uuid, uuid, timestamptz, timestamptz, uuid) IS
  'Item Sales summary: qty sold/refunded, gross/net, COGS per product+variant; optional category filter.';
