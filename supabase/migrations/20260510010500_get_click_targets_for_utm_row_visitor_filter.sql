-- UTM row click details can now be scoped by visitor_id. This matches the
-- visitor-based row grouping in get_traffic_dashboard while preserving the
-- existing session_id fallback for old rows.
    ...
    FROM (
      ...
      WHERE ce.web_id = p_web_id
        ...
        AND (
          (
            v_visitor_id <> ''
            AND bx.visitor_id = v_visitor_id
          )
          OR (
            v_visitor_id = ''
            AND v_session_id <> ''
...
REVOKE ALL ON FUNCTION public.get_click_targets_for_utm_row(
  text, date, date, text, text, text, text, text, text, text, date, int, text
) FROM PUBLIC;
...
COMMENT ON FUNCTION public.get_click_targets_for_utm_row(
  ...
  'Click targets for UTM table row; visitor_id is preferred, with session_id fallback.';
