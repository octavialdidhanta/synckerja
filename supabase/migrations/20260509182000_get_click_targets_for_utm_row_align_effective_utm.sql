-- Detail klik UTM: route + dimensi UTM sama seperti refresh_analytics_daily_rollups (effective + landing URL).
-- Hapus syarat started_at sesi = hari klik (rollup mengaitkan klik per hari WIK ke bucket session-day lain).
...
COMMENT ON FUNCTION public.get_click_targets_for_utm_row(
  ...
  'Click targets for UTM table row; route + UTM dims match refresh_analytics_daily_rollups (effective UTM, landing path order).';
