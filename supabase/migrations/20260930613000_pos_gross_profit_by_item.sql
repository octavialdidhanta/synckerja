-- Gross profit item breakdown RPC
...
REVOKE ALL ON FUNCTION public.pos_gross_profit_by_item(uuid, uuid, timestamptz, timestamptz) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.pos_gross_profit_by_item(uuid, uuid, timestamptz, timestamptz) TO authenticated;
GRANT EXECUTE ON FUNCTION public.pos_gross_profit_by_item(uuid, uuid, timestamptz, timestamptz) TO service_role;

COMMENT ON FUNCTION public.pos_gross_profit_by_item(uuid, uuid, timestamptz, timestamptz) IS
  'Per-product/variant gross profit for Store Checkout Converted (non-refunded) in window.';
