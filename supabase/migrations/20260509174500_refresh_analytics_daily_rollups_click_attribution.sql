-- Atribusi klik Sumber traffic / UTM selaras Top pages: semua analytics_click_events dihitung
-- dengan source_key dari analytics_sessions (last-touch + URL), tanpa mewajibkan human_sessions.
-- Sesi & page view tetap dari human_sessions; scroll tetap dari classified × page_views.
...
COMMENT ON FUNCTION public.refresh_analytics_daily_rollups(date, date, text) IS
  'Rebuild analytics_daily_source_breakdown + analytics_daily_utm; sessions/PV/scroll from human_sessions; clicks from all click events with same source rules as sessions (last-touch + URL).';
