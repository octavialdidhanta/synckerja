-- get_traffic_dashboard: add `source_breakdown` — sessions grouped by coarse acquisition channel
-- (UTM vs paid click ids vs referral vs direct) so totals reconcile with KPI sessions vs UTM table.
  ...
  FROM (
    ...
    FROM (
      SELECT
        ...
        SUM(COALESCE(pv.cnt, 0::bigint))::bigint AS page_views,
        SUM(COALESCE(ce.cnt, 0::bigint))::bigint AS clicks
      FROM (
        SELECT
          s.id,
          CASE
      ...
      LEFT JOIN (
        ...
        WHERE pv.web_id = p_web_id
          AND (pv.started_at AT TIME ZONE 'UTC')::date BETWEEN v_from AND v_to
        GROUP BY session_id
      ) pv ON pv.session_id = x.id
      LEFT JOIN (
        SELECT session_id, COUNT(*)::bigint AS cnt
        FROM public.analytics_click_events ce
        WHERE ce.web_id = p_web_id
          AND (ce.created_at AT TIME ZONE 'UTC')::date BETWEEN v_from AND v_to
        GROUP BY session_id
      ) ce ON ce.session_id = x.id
      GROUP BY x.src_key
    ) q
  ) agg;
