-- get_traffic_dashboard: add click detail fields per top page (uses analytics_daily_top_click_targets).
...
BEGIN
  ...
  -- Top pages with derived metrics + (top) click element detail
  WITH
    base AS (
      SELECT
        tp.path,
