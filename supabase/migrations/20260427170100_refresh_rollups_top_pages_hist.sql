-- refresh_analytics_rollups: fill analytics_daily_top_pages.active_ms_hist per (web_id, day, path).
...
BEGIN
  ...
  WHILE d <= v_to LOOP
    ...
    -- Build histograms for each (path) inserted above.
    WITH
      paths AS (
        SELECT tp.path
    ...
    -- Source breakdown rollup (event-day attribution) (unchanged)
    WITH
      src_keys AS (
