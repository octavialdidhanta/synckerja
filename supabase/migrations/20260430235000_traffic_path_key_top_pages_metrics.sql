-- 1) Normalisasi string path agar rollup `analytics_daily_top_pages` (GROUP BY path)
--    match row di `analytics_page_views` walaupun tracker kirim '', '/', casing, trailing slash.
-- 2) Median & scroll top pages diaggregasi per *path tampilan* (b.path) dari raw, bukan GROUP BY pv.path
--    (hindari miss join ke baris `base`).

CREATE OR REPLACE FUNCTION public.traffic_path_key(p text)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
AS $fn$
...
REVOKE ALL ON FUNCTION public.traffic_path_key(text) FROM PUBLIC;
COMMENT ON FUNCTION public.traffic_path_key(text) IS
  'Canonical path key for matching analytics daily rollup rows to raw analytics_page_views.';

-- Replace get_traffic_dashboard: top_pages median/scroll + path_key
...
BEGIN
  ...
  -- Top pages: ranking dari tabel harian; median + scroll dari raw, join lewat traffic_path_key
  WITH
    base AS (
      SELECT
        tp.path,
