-- Correct UTM Tracking back to one row per session_id.
-- Clicks are summed per session across the selected range, while row membership follows Source traffic sessions.
...
COMMENT ON FUNCTION public.get_traffic_dashboard(text, date, date, int, int, int) IS
  'Dashboard RPC wrapper; UTM table is one row per source traffic session with clicks summed across the selected range.';
