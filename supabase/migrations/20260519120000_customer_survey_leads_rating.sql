-- Customer survey: latest/history RPCs for leads table; recipient picker survey rating filter + fields.

-- ---------------------------------------------------------------------------
-- Latest survey per WhatsApp conversation (batch for leads table)
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.customer_survey_latest_by_conversations(
...
DECLARE
  v_allowed boolean;
BEGIN
  IF p_organization_id IS NULL OR p_conversation_ids IS NULL OR cardinality(p_conversation_ids) = 0 THEN
    RETURN '[]'::jsonb;
  END IF;
  ...
  RETURN coalesce(
    (
      SELECT jsonb_agg(
        jsonb_build_object(
          'conversation_id', x.whatsapp_conversation_id,
          'rating', x.rating,
          'comment', x.comment,
          'submitted_at', x.submitted_at,
          'assignee_id', x.assignee_id,
          'assignee_name', coalesce(nullif(btrim(e.full_name), ''), nullif(btrim(e.email), ''))
        )
      ...
      FROM (
        ...
        WHERE r.organization_id = p_organization_id
          AND r.whatsapp_conversation_id = ANY (p_conversation_ids)
        ORDER BY r.whatsapp_conversation_id, r.submitted_at DESC
      ) x
      LEFT JOIN public.employees e ON e.id = x.assignee_id
    ),
    '[]'::jsonb
  );
END;
...
COMMENT ON FUNCTION public.customer_survey_latest_by_conversations(uuid, uuid[]) IS
  'Latest customer_survey_responses per whatsapp_conversation_id for active org members.';

GRANT EXECUTE ON FUNCTION public.customer_survey_latest_by_conversations(uuid, uuid[]) TO authenticated;

-- ---------------------------------------------------------------------------
-- Full survey history for one conversation (modal)
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.customer_survey_history(p_conversation_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY INVOKER
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    FROM public.profiles p
    WHERE p.user_id = auth.uid()
      AND p.active_organization_id = v_org_id
  )
  INTO v_allowed;
  ...
  RETURN coalesce(
    (
      ...
      WHERE r.whatsapp_conversation_id = p_conversation_id
...
COMMENT ON FUNCTION public.customer_survey_history(uuid) IS
  'All survey responses for a WhatsApp conversation, newest first.';

GRANT EXECUTE ON FUNCTION public.customer_survey_history(uuid) TO authenticated;

-- Patch search_whatsapp_recipient_picker: surveyRating filter + latest_survey_* on items.

-- Recipient picker: eff_status uses whatsapp_conversations.meta_session_expires_at (Meta) + lead_statuses Expired, not server 24h interval.
-- Requires 20260517130000_meta_session_expired_core.sql (column meta_session_expires_at + Expired seed).
      ...
      CASE lower(
        CASE
          WHEN lower(btrim(coalesce(f.j ->> 'sortColumn', ''))) IN (
            ...
            'utm_source', 'utm_campaign', 'utm_medium', 'utm_content', 'utm_term', 'landing_url', 'attribution_label',
            'assignee', 'followup', 'fu_priority', 'status'
          )
        ...
        WHEN 'survey_rating' THEN lpad(coalesce(d.latest_survey_rating::text, ''), 2, '0')
...
'JSON {total, items[]} for WhatsApp recipient picker; eff_status uses Meta meta_session_expires_at + Expired status (not server 24h); optional sortColumn/sortDir.';
