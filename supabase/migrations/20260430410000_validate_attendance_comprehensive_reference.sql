-- Replace stub validate_attendance_comprehensive(uuid, uuid, jsonb) with reference-aligned RPC.
-- PostgREST + clients expect Args: employee_id_param, organization_id_param, latitude_param,
-- longitude_param, face_image_data (synckerja-reference types.ts validate_attendance_comprehensive).
-- Returns a single row matching reference Return fields (array of one row over REST).

DROP FUNCTION IF EXISTS public.validate_attendance_comprehensive(uuid, uuid, jsonb);
