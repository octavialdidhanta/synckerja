-- Align pos_gross_profit_by_item net_sales with summary checkout_subtotal
-- by allocating bill-level discount proportionally across line gross amounts.
    ...
    SELECT
      ...
      CASE
        WHEN ag.lines_gross > 0 AND sa.checkout_subtotal IS NOT NULL THEN
          COALESCE(sai.total_price, 0) * (COALESCE(sa.checkout_subtotal, 0) / ag.lines_gross)
