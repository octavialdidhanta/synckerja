-- Per-row UTM rollup: attribute page views & clicks to sessions (same route + utm_* bucket as sessions_count).
-- Enables dashboard KPIs to follow UTM table filters (sum filtered rows) while reset uses global kpis.
