-- Allow "Maximum" / all-time range: p_from and/or p_to may be NULL.
-- NULL means derive bounds from raw analytics tables for the given web_id (UTC calendar dates).
...
BEGIN
  ...
  IF v_min IS NULL OR v_max IS NULL THEN
    -- No raw rows for this web_id; nothing to rollup.
    RETURN;
  END IF;
