-- Recipient picker: distinct gclid + service_names for column header filters.
...
COMMENT ON FUNCTION public.recipient_picker_filter_options(uuid) IS
  'Distinct filter option values for recipient picker (org owner only), including gclid and service_names.';


-- Re-apply search with gclid column filter (exact match).
