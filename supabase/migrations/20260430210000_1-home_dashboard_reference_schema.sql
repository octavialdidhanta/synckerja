...
-- training_participants has no organization_id; scope via training_programs
ALTER TABLE public.training_participants ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "training_participants_org" ON public.training_participants;
CREATE POLICY "training_participants_org"
  ...
  USING (
    EXISTS (
      ...
      WHERE tp.id = training_participants.training_program_id
        AND tp.organization_id IN (SELECT public.user_organization_ids())
    )
  )
  WITH CHECK (
    EXISTS (
