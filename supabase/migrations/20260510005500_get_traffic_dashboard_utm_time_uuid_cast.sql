-- Fix UTM Tracking time wrapper when analytics session_id columns are uuid.
  ...
  WITH
    ...
    session_times AS (
      ...
      FROM (
        ...
        SELECT
          ce.session_id::text AS session_id,
...
COMMENT ON FUNCTION public.get_traffic_dashboard(text, date, date, int, int, int) IS
  'Dashboard RPC wrapper; UTM table includes minute-level first-seen time per session with uuid-safe session matching.';
