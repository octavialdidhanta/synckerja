-- Fix erp_expense bank_statement_lines.transaction_date: use paid_at / disbursement completed_at
-- instead of create_date + 12h (which always showed 19:00 WIB).
...
COMMENT ON FUNCTION public.upsert_bank_statement_from_erp_expense(uuid) IS
  'Mirror Payment Process expense as bank_statement_lines debit. transaction_date uses paid_at / disbursement completed_at.';

-- Backfill rows that used the old create_date + 12h placeholder.
UPDATE public.bank_statement_lines sl
SET transaction_date = sub.resolved_at
FROM (
  SELECT
    sl2.id AS line_id,
    COALESCE(
      ...
      e.created_at,
      sl2.transaction_date
    ) AS resolved_at
  FROM public.bank_statement_lines sl2
  JOIN public.expenses e ON e.id = sl2.expense_id
  JOIN public.purchase_requests pr ON pr.id = e.purchase_request_id
  WHERE sl2.origin = 'erp_expense'
    AND sl2.transaction_date = (e.create_date::timestamptz + interval '12 hours')
) sub
WHERE sl.id = sub.line_id;
