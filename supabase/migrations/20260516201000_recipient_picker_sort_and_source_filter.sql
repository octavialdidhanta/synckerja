-- Recipient picker: optional p_filters.sortColumn / p_filters.sortDir (whitelist; default client asc; tie-break phone_key).
-- Source filter matches livechat rows (WhatsApp). recipient_picker_filter_options adds distinct `sources` (leads + WhatsApp when WA convs exist).
...
COMMENT ON FUNCTION public.recipient_picker_filter_options(uuid) IS 'Distinct filter option values for recipient picker (org owner only), including lead/livechat sources.';
...
'JSON {total, items[]} for WhatsApp recipient picker; optional p_filters.sortColumn/sortDir (whitelist); source filter includes livechat rows; see migration header.';
