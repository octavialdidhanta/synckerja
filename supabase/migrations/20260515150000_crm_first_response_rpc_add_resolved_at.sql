-- Add resolved_at to CRM room-cycle RPC (same row set as first-response table; avoids duplicate fetch).
...
COMMENT ON FUNCTION public.get_crm_first_response_time_per_room(uuid) IS
  'Latest conversation cycle per room: cycle start, first reply, resolution instant, customer, current assignee. SLA columns reserved (null).';
