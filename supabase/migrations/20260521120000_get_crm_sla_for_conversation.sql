-- Single-conversation SLA snapshot for livechat Quick Action (no full-org scan).
-- Same semantics as get_crm_first_response_time_per_room: first-reply due from first_assignee_in_cycle_at + policy;
-- resolution due from first_response_at + resolution minutes. Uses now() for open cycles (VOLATILE).
...
COMMENT ON FUNCTION public.get_crm_sla_for_conversation(uuid, uuid, text) IS
  'Latest cycle SLA snapshot for one omnichannel conversation. First-reply due from first_assignee_in_cycle_at + policy; resolution due from first_response_at. Channel: whatsapp|instagram|email (normalized). Empty if no cycle row.';
