-- Top pages: median + scroll dihitung dari `analytics_page_views` (filter hari WIB), bukan
-- hanya dari `analytics_daily_top_pages.active_ms_hist` / scroll_*.
-- Sumber traffic sudah punya scroll (analytics_daily_source_breakdown) dari raw yang sama;
-- bila tabel per-path harian belum terisi hist/scroll, top pages tetap bisa menampilkan metrik.
--
-- Jika join `pv.path = b.path` tidak match (contoh: '' vs '/'), terapkan juga
-- `20260430235000_traffic_path_key_top_pages_metrics.sql` (traffic_path_key + get_traffic penuh).
...
BEGIN
  ...
  -- Top pages: peringkat dari tabel harian; median & scroll dari raw (satu sumber dengan sumber traffic)
  WITH
    base AS (
      SELECT
        tp.path,
    ...
    raw_median_by_path AS (
      ...
      INNER JOIN base b ON b.path = pv.path
    ...
    raw_scroll_by_path AS (
      SELECT
        sp.path,
        MAX(sp.sp_max) FILTER (WHERE sp.sp_max IS NOT NULL)::double precision AS max_deep_scroll_pct,
        CASE
          WHEN COUNT(*) FILTER (WHERE sp.sp_max IS NOT NULL) = 0 THEN NULL
          ELSE
            (SUM(sp.sp_max) FILTER (WHERE sp.sp_max IS NOT NULL) /
              NULLIF(COUNT(*) FILTER (WHERE sp.sp_max IS NOT NULL), 0))::double precision
  ...
  LEFT JOIN raw_median_by_path rm ON rm.path = b.path
  LEFT JOIN click_targets_agg ct ON ct.path = b.path
  LEFT JOIN raw_scroll_by_path rs ON rs.path = b.path;
