-- Align path filter with get_traffic_dashboard / traffic_path_key so detail klik matches
-- analytics_daily_top_click_targets rows walaupun casing / home path berbeda di rollup.
  ...
  RETURN (
    ...
    FROM (
      ...
      WHERE t.web_id = p_web_id
        ...
        AND public.traffic_path_key(t.path) = public.traffic_path_key(p_path)
...
COMMENT ON FUNCTION public.get_click_targets_for_path(text, date, date, text, int) IS
  'Click target breakdown for a page path; paths matched via traffic_path_key (same as traffic dashboard).';
