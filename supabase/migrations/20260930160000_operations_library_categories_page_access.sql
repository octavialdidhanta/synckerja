-- Operations Library: categories page at /operations/library/categories.
...
WHERE p.page_path = '/operations/library/product-list'
  AND NOT EXISTS (
    ...
    WHERE existing.organization_id = p.organization_id
      AND existing.page_path = '/operations/library/categories'
  );
...
WHERE d.page_path = '/operations/library/categories'
  AND NOT EXISTS (
    SELECT 1
    FROM public.permission_configurations p
