-- Recipient picker: one row per CRM lead / orphan livechat (not per phone).
-- Includes all org leads (even without resolvable WA phone) so the picker matches /omnichannel/leads.
-- Re-apply search_whatsapp_recipient_picker (lead_submissions profile, filters, survey sort).
