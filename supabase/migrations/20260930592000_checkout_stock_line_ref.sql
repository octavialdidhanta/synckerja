-- Checkout stock movements: unique reference_id per cart line index (L{i})
-- so two lines of the same product (different modifiers) do not collide.
  ...
  LOOP
    ...
    IF v_tracked IS TRUE THEN
      PERFORM public.apply_catalog_stock_movement(
        ...
        -v_qty,
        'store_checkout',
        p_activity_id || ':' || v_line_key || ':' || v_product::text,
        'POS sale'
      );
    END IF;
    ...
    LOOP
      PERFORM public.apply_catalog_stock_movement(
        ...
        'store_checkout',
        p_activity_id || ':' || v_line_key || ':' || v_product::text || ':ing:' || v_recipe.ingredient_id::text,
        'Recipe consume'
      );
    END LOOP;
...
COMMENT ON FUNCTION public.apply_catalog_checkout_stock(uuid, uuid, text, jsonb) IS
  'Deduct finished goods when track_stock; base recipe + stock_enabled modifier BOM. reference_id includes L{lineIndex} for multi-line same product.';
