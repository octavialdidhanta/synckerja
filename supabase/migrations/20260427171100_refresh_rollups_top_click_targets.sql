-- refresh_analytics_rollups: populate analytics_daily_top_click_targets (includes target_url).
...
BEGIN
  ...
  d := v_from;
  WHILE d <= v_to LOOP
    -- sessions / page views / clicks (overall)
    ...
    -- top click targets (new; includes target_url)
    ...
    -- UTM + source breakdown are unchanged from the latest definition
    -- (keep existing sections to avoid drift)
    ...
    INSERT INTO public.analytics_daily_source_breakdown (
      web_id, day, source_key, sessions_count, page_views_count, clicks_count
    )
    SELECT
      p_web_id,
      d,
