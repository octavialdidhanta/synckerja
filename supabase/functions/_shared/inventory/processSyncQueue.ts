...
import { createTikTokShopStockAdapter } from "../marketplace-adapters/tiktokShopStockAdapter.ts";
import {
  blibliStockAdapter,
  shopeeStockAdapter,
  tokopediaStockAdapter,
} from "../marketplace-adapters/shopeeStockAdapter.ts";
import type { MarketplaceStockAdapter } from "../marketplace-adapters/types.ts";
...
export async function processInventorySyncQueue(
  admin: SupabaseClient,
  batchSize = 20,
  ...
  for (const job of jobs ?? []) {
    ...
    await admin.from("inventory_sync_queue").update({ status: "processing" }).eq("id", jobId);
