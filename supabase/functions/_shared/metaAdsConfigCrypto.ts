/**
 * AES-256-GCM encrypt/decrypt for per-org Meta Ads long-lived access tokens.
 * Key: META_ADS_CONFIG_ENCRYPTION_KEY (32-byte base64 or 64-char hex).
 */
...
function decodeKey(raw: string): Uint8Array {
  ...
  if (bytes.length !== 32) {
    throw new Error("META_ADS_CONFIG_ENCRYPTION_KEY must be 32 bytes (base64 or 64-char hex)");
  }
  return bytes;
}

function getKeyBytes(): Uint8Array {
  const raw = Deno.env.get("META_ADS_CONFIG_ENCRYPTION_KEY") ?? "";
...
export async function encryptMetaAdsAccessToken(plaintext: string): Promise<string> {
...
export async function decryptMetaAdsAccessToken(ciphertextB64: string): Promise<string> {
  const keyBytes = getKeyBytes();
