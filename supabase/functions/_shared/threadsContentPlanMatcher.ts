...
export type ThreadsPostMatchInput = {
  id: string;
  permalink: string | null;
};

export function parseThreadsPostIdFromUrl(url: string): string | null {
...
export async function loadThreadsPlanLinks(
  ...
  const rows = (links ?? []).filter((l) => {
    ...
    return platform.includes("threads");
  }) as LinkRow[];
...
export function buildPlanMatchIndex(links: LinkRow[]): {
  ...
  for (const link of links) {
    const postId = link.external_post_id?.trim() || parseThreadsPostIdFromUrl(link.url);
  ...
  for (const m of matches) {
    await admin
      ...
      .update({
        ...
        platform_account_open_id: threadsUserId,
