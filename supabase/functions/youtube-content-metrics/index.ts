...
Deno.serve(async (req: Request) => {
  try {
    ...
    const links = await loadYouTubePlanLinks(admin, organizationId);
    ...
    const rows = videos.map((video) => {
      ...
      const postedAt = video.published_at ? new Date(video.published_at).toISOString() : null;
    ...
    if (backfillMatches.length > 0) {
      await backfillLinkVideoIds(admin, organizationId, channelId, backfillMatches);
    }
    ...
    console.error("youtube-content-metrics unhandled:", msg);
