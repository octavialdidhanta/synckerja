...
export const ChangePasswordModal = ({ open, onOpenChange }: ChangePasswordModalProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent
        className={cn(
          'z-30 m-0 flex w-full max-w-none translate-x-0 translate-y-0 flex-col gap-0 rounded-none p-0',
          'fixed left-0 right-0 top-0 border border-brand-blue/20 bg-gradient-to-b from-brand-blue-soft/55 via-gray-100 to-gray-100 modal-above-safe-area dark:from-brand-blue/20 dark:via-background dark:to-muted/80',
          'sm:left-[50%] sm:top-[50%] sm:h-auto sm:min-h-0 sm:max-h-[90vh] sm:max-w-md sm:translate-x-[-50%] sm:translate-y-[-50%] sm:rounded-lg sm:border-border sm:bg-background',
        )}
        overlayClassName="!z-30"
        fullscreenAnimation={isMobile}
        hideCloseButton
      >
        <DialogHeader className="safe-area-top flex flex-shrink-0 flex-row flex-nowrap items-stretch gap-0 space-y-0 border-b border-brand-blue/20 bg-gradient-to-r from-brand-blue-soft via-background to-brand-blue-soft/70 px-0 py-0 text-left dark:from-brand-blue/15 dark:via-background dark:to-brand-blue/10 sm:border-border sm:bg-background sm:from-background sm:via-background sm:to-background">
          <div className="flex w-full min-w-0 items-center gap-1.5 px-3 py-2 sm:px-6 sm:py-3">
            <button
              type="button"
        ...
        <form onSubmit={handleChangePassword} className="flex flex-col flex-1 min-h-0">
          ...
          <div className="flex-shrink-0 border-t border-brand-blue/15 bg-brand-blue-soft/25 px-4 pb-3 pt-3 dark:border-brand-blue/25 dark:bg-brand-blue/10 sm:border-border sm:bg-muted/30">
