...
import { ExpenseTableFooter } from "@/4-2-dashboard/components/ExpenseTableFooter";
...
async function openExpenseReceiptFile(filePath: string | null | undefined, preferPurchaseBucket: boolean) {
  if (!filePath) {
...
export function ExpenseTableSection({ expenseTable }: ExpenseTableSectionProps) {
  ...
  const isPurchaseRow = (e: ExpenseTableItem) => paidPurchaseRequests.some((pr) => pr.id === e.id);

  useEffect(() => {
    if (!isDetailModalOpen || !selectedExpense?.receipt_url) {
  ...
  }, [isDetailModalOpen, selectedExpense, paidPurchaseRequests]);
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-40">
                              {expense.receipt_url && (
                                <DropdownMenuItem
                                  onClick={() =>
                                    void openExpenseReceiptFile(expense.receipt_url, isPaidPurchaseRequest)
                                  }
                                >
                      ...
                      <div>
                        ...
                        <div className="mt-2 space-y-2">
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              void openExpenseReceiptFile(
                                selectedExpense.receipt_url,
                                isPurchaseRow(selectedExpense),
                              )
                            }
                          >
                            <Receipt className="h-4 w-4 mr-2" />
                          ...
                          {!/\.pdf$/i.test(selectedExpense.receipt_url) ? (
                            <div className="w-full min-h-[calc(100vh-12rem)] rounded-md border bg-muted/30 overflow-hidden">
                              {receiptPreviewUrl ? (
                                ...
                                <div
                                  className="min-h-[200px] bg-muted/25"
                                  aria-busy="true"
