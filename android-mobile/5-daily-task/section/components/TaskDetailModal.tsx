...
import type { Task, TaskStep as TaskStepEntity } from '@/8-2-DailyTask/context/DailyTaskContext';
import { getEffectiveProgressAndCount } from '@/8-2-DailyTask/utils/taskUtils';
  ...
  return (
    <Dialog open onOpenChange={handleOpenChange}>
      ...
      >
        ...
        <div className="safe-area-top flex flex-shrink-0 flex-row flex-nowrap items-stretch gap-0 space-y-0 border-b border-border px-0 py-0">
          <div className="flex w-full min-w-0 items-center gap-1.5 px-3 py-2">
            <button
              type="button"
              onClick={handleClose}
              className="-ml-0.5 inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full p-0 transition-colors hover:bg-muted"
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              <span
                className={`m-0 truncate text-base font-semibold leading-tight ${
                  isTaskCompletedVisual ? 'line-through text-muted-foreground' : 'text-foreground'
                }`}
              >
                {task.title}
              </span>
              <span className="mt-0.5 truncate text-xs leading-tight text-muted-foreground">{displayProgress}% complete</span>
            </div>
          </div>
        </div>

        <div
          className={`scrollbar-hide seamless-scroll flex-1 min-h-0 overflow-y-auto overflow-x-hidden py-3 space-y-3 min-w-0 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden ${isMobile ? 'px-2' : 'px-4'}`}
          style={{
              ...
              <div className="space-y-1">
                {visibleSteps.length === 0 ? (
                  ...
                  visibleSteps
                    ...
                    .map((step, index) => (
                      /** Fullscreen task detail is always swipe-to-reveal steps; do not branch on `useIsMobile` (wide web / SSR could render desktop TaskStep with icons always visible). */
                      <MobileTaskStep
                        key={step.id}
                        step={step}
                        index={index}
