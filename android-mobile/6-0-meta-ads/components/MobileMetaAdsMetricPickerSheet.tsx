...
export function MobileMetaAdsMetricPickerSheet({
  open,
  onOpenChange,
  selectedKey,
  onSelectKey,
  ...
  const groups = useMemo(() => metaAdsSummaryMetricGroups(options), [options]);
      ...
      <DrawerContent className="max-h-[85vh] px-0 pb-4">
        <DrawerHeader className="px-4 pb-2 text-left">
          <DrawerTitle className="text-base">
            {t("digitalMarketing.metaAds.metricsButton", "Columns")}
          </DrawerTitle>
        </DrawerHeader>
        <Command className="rounded-none border-0 bg-transparent">
          ...
          <CommandList className="max-h-[min(60vh,360px)] px-1">
            <CommandEmpty>
              {t("digitalMarketing.metaAds.summaryNoMetrics", "No metrics found.")}
            </CommandEmpty>
