...
export function LeadsReportSummaryView() {
  ...
  const { leads, initialLoadPending, refetch } = useLeads({ scope: 'all' });
  const { data: employees = [], isLoading: employeesLoading } = useAvailableEmployees();
  const { organizationId, loading: orgLoading } = useCurrentOrg();
  const { insightsSupplementalPending } = useLeadsInsightsSupplementalQueries(organizationId);
  const dataPending =
    orgLoading ||
    initialLoadPending ||
    (!!organizationId && employeesLoading) ||
  ...
  /** Sembunyikan konten live di bawah overlay skeleton; Fragment = header & scroll sebagai anak langsung `main` (mobile-tools-layout-android.mdc §1–§2). */
  const suppressLiveContent = showPageSkeleton ? 'pointer-events-none invisible select-none' : '';
    ...
    <>
      ...
      >
        <div
          ref={listScrollRef}
          className={cn(
            'flex min-h-0 flex-1 flex-col overflow-x-hidden overflow-y-auto seamless-scroll',
            'scrollbar-hide nested-scroll-touch-chain touch-pan-y overscroll-contain',
      ...
      {showPageSkeleton ? <MobileConsultantLeadsReportPageSkeletonOverlay /> : null}
    </>
  );
}
