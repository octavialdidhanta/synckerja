...
interface BillsTotalRecurringCardProps {
  count: number;
  amount: number;
}

export function BillsTotalRecurringCard({ count, amount }: BillsTotalRecurringCardProps) {
      ...
      <CardContent className="flex min-w-0 flex-col gap-3 p-3">
        <div className="flex items-center gap-2">
          ...
          <span className="truncate text-sm font-medium text-white/90">
            {t("reminderBills.metrics.totalRecurring", "Total Recurring Bills")}
          </span>
        </div>
