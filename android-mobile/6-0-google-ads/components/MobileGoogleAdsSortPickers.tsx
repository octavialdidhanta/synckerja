...
/**
 * Compact sort field + direction triggers (Impr. | High → low), drawer pickers.
 * Uses shared {@link googleAdsSortColumns} helpers — same labels as desktop.
 */
export function MobileGoogleAdsSortPickers({
  sort,
  sortColumnOptions,
  entity,
  metricItems,
  ...
  const fieldLabel =
    sortColumnOptions.find((o) => o.key === sortFieldValue)?.label ?? sortFieldValue;
  ...
  return (
    <div className={cn("flex shrink-0 items-center gap-2", className)}>
      <Button
        ...
        aria-label={t("digitalMarketing.googleAds.sortBy", "Sort by")}
      >
      ...
      <Button
        ...
        aria-label={t("digitalMarketing.googleAds.sortDirection", "Sort direction")}
      >
      ...
      <Drawer open={directionOpen} onOpenChange={setDirectionOpen}>
        <DrawerContent className="max-h-[85vh] px-0 pb-4">
          <DrawerHeader className="px-4 pb-2 text-left">
            <DrawerTitle className="text-base">
              {t("digitalMarketing.googleAds.sortDirection", "Sort direction")}
            </DrawerTitle>
          </DrawerHeader>
