...
export function PiutangRemainingCard({ totalRemaining, activityCount }: Props) {
      ...
      <CardContent className="flex min-w-0 flex-col gap-3 p-3">
        <div className="flex items-center gap-2">
          ...
          <span className="truncate text-sm font-medium text-white/90">
            {t('incomes.piutang.metrics.totalRemaining', 'Total sisa piutang')}
          </span>
        </div>
        <div className="min-w-0 flex flex-1 items-center justify-between gap-3">
          <div className="min-w-0 flex-1">
            <div className="truncate text-2xl font-bold text-white sm:text-3xl">
              {formatToRupiah(totalRemaining)}
            </div>
            <div className="mt-1 truncate text-xs text-white/80">
              {activityCount} {t('incomes.piutang.metrics.activitiesInFilter', 'aktivitas (filter)')}
            </div>
          </div>
        </div>
      </CardContent>
