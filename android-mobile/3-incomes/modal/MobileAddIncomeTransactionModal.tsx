...
import { useIncomeTransactions, useIncomeMasterData } from "@/4-1-dashboard/hooks";
...
import type { CreateIncomeTransactionData } from "@/4-1-dashboard/types";
import { useAppTranslation } from "@/shared/i18n/useAppTranslation";
import { CameraModal } from "@/mobile-app/components/CameraModal";
import { MobileIncomeTransactionDateField } from "@/mobile/3-incomes/components/MobileIncomeTransactionDateField";
...
export interface MobileAddIncomeTransactionModalProps {
  open: boolean;
...
export function MobileAddIncomeTransactionModal({
  ...
  const selectedIncomeType = useMemo(
    () => incomeTypes.find((ty) => ty.id === watchedIncomeTypeId),
  ...
  const onSubmit = (data: FormData) => {
    ...
    const submitIncomeType = incomeTypes.find((ty) => ty.id === data.income_type_id);
  ...
  return (
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      >
        ...
        <div
          ref={scrollBodyRef}
          className="scrollbar-hide seamless-scroll flex-1 min-h-0 overflow-y-auto overflow-x-hidden px-4 py-4 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
        >
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-2">
                <Label htmlFor="payment_method">{t("incomes.paymentMethodLabel", "Payment Method")}</Label>
                <DrawerSelectField
                  open={paymentMethodDrawerOpen}
                  onOpenChange={setPaymentMethodDrawerOpen}
                  title={t("incomes.paymentMethodLabel", "Payment Method")}
              ...
              <div className="space-y-2">
                ...
                <DrawerSelectField
                  ...
                  onSelect={(value) => {
                    ...
                    const next = incomeTypes.find((ty) => ty.id === value);
