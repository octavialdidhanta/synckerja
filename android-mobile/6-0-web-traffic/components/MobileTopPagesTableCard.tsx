...
export function MobileTopPagesTableCard({
  rows,
  onClickClicks,
}: {
  rows: TopPageRow[];
    ...
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
      <div className="shrink-0 border-b border-gray-200 px-4 py-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-2 text-xs text-gray-500">
            <span className="whitespace-nowrap">{topPagesNonBlog.length} items</span>
          </div>
        </div>
      </div>
      ...
      {topPagesNonBlog.length === 0 ? (
        ...
        <div
          ...
          className="scrollbar-hide seamless-scroll nested-scroll-touch-chain flex-1 min-h-0 overflow-x-auto overflow-y-auto touch-pan-x overscroll-x-contain [-webkit-overflow-scrolling:touch] [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
