...
import { getSocialMediaPerformanceMobileHeader } from "@/6-0-social-media-performance/shared/socialMediaPerformanceTabs";

type SocialMediaPerformanceMobileShellHeaderProps = {
  onRefresh?: () => void;
...
export function SocialMediaPerformanceMobileShellHeader({
  onRefresh,
  refreshDisabled,
  isRefreshing,
  headerActions,
}: SocialMediaPerformanceMobileShellHeaderProps) {
  const { t } = useAppTranslation();
  const { pathname } = useLocation();
  const header = getSocialMediaPerformanceMobileHeader(pathname);
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2">
          ...
          <div className="min-w-0">
            <h1 className="truncate text-base font-semibold leading-tight text-foreground">
              {t(header.titleKey, header.titleDefault)}
            </h1>
          </div>
        </div>
