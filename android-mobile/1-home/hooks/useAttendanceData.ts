...
export const useAttendanceData = () => {
  ...
  /** After first completed fetch; refetches keep prior UI (no full-page loading / blank flash). */
  const initialFetchDoneRef = useRef(false);
  /** Last successfully loaded org; used to detect org switch and reset state. */
  ...
  const fetchAttendanceData = async () => {
    try {
      ...
      const newOrgId = profile.active_organization_id;
      const orgSwitched =
        loadedOrgIdRef.current != null && loadedOrgIdRef.current !== newOrgId;
      const showGlobalLoading = !initialFetchDoneRef.current || orgSwitched;

      if (orgSwitched) {
        logger.debug("Organization changed from", loadedOrgIdRef.current, "to", newOrgId);
      }
      ...
      loadedOrgIdRef.current = newOrgId;
    } catch (err) {
