...
const PAYMENT_TABLE_COLUMNS = 11;
const PAYMENT_TABLE_BODY_ROWS = 10;

const MOBILE_PAYMENT_TABLE_VIEWPORT_CLASS =
  "h-[min(28rem,calc(100dvh-14rem))] max-h-[28rem] min-h-[11rem] shrink-0";

const TABLE_SCROLL_HIDE =
  "scrollbar-hide [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden";

export function MobilePaymentCarouselSkeleton() {
  return (
...
export function MobilePaymentTableBodyRows() {
  return (
    <>
      {Array.from({ length: PAYMENT_TABLE_BODY_ROWS }).map((_, rowIndex) => (
        <tr key={rowIndex} className="border-b border-border">
          {Array.from({ length: PAYMENT_TABLE_COLUMNS }).map((__, ci) => (
...
function MobilePaymentTableCardSkeleton() {
  return (
      ...
      <CardContent className="flex min-h-0 min-w-0 flex-col p-0">
        ...
        <div
          className={cn(
            ...
            TABLE_SCROLL_HIDE,
            MOBILE_PAYMENT_TABLE_VIEWPORT_CLASS,
          )}
        >
          <table className="min-w-[1400px] w-full border-collapse text-sm caption-bottom">
            <thead className="sticky top-0 z-10 border-b border-border bg-muted">
              <tr>
                {Array.from({ length: PAYMENT_TABLE_COLUMNS }).map((_, i) => (
...
export type MobilePaymentProcessPageSkeletonProps = {
  className?: string;
};

export function MobilePaymentProcessPageSkeleton({ className }: MobilePaymentProcessPageSkeletonProps) {
  return (
    <div className={cn("flex min-h-0 min-w-0 flex-1 flex-col gap-1", className)}>
      <div className="shrink-0">
        <MobilePaymentCarouselSkeleton />
      </div>
      <div className="flex min-h-0 min-w-0 shrink-0 flex-col">
        <MobilePaymentTableCardSkeleton />
      </div>
    </div>
  );
}

export type MobilePaymentChromeSkeletonProps = {
  wrapperClassName?: string;
};

export function MobilePaymentChromeSkeleton({ wrapperClassName }: MobilePaymentChromeSkeletonProps) {
  const { t } = useAppTranslation();
  const aria = t("payments.route.loadingAria", "Loading payment process");
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        <div className="scrollbar-hide flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden overflow-y-auto seamless-scroll [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <div className="flex min-h-full min-w-0 flex-1 flex-col">
            ...
            <MobilePaymentProcessPageSkeleton className="mx-auto flex min-h-0 w-full max-w-md flex-1 flex-col content-padding-above-nav-default px-2 pt-2" />
          </div>
        </div>
      </div>
...
export function MobilePaymentProcessShellSkeleton() {
  return (
    <div className="flex min-h-screen w-full bg-background">
      <MobilePaymentChromeSkeleton />
    </div>
  );
}

export function MobilePaymentProcessFullViewportOverlay() {
  return (
    <MobilePaymentChromeSkeleton wrapperClassName="fixed inset-0 z-[200] min-h-[100dvh] bg-background" />
  );
}
