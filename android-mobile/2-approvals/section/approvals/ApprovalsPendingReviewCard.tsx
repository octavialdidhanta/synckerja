...
interface ApprovalsPendingReviewCardProps {
  count: number;
  pendingAmount: number;
}

export function ApprovalsPendingReviewCard({ count, pendingAmount }: ApprovalsPendingReviewCardProps) {
      ...
      <CardContent className="flex min-w-0 flex-col gap-3 p-3">
        <div className="flex items-center gap-2">
          ...
          <span className="truncate text-sm font-medium text-white/90">
            {t("approvals.metrics.pendingReview", "Pending Review")}
          </span>
        </div>
        <div className="flex min-w-0 items-center justify-between gap-3">
          <div className="min-w-0 flex-1">
            <div className="truncate text-2xl font-bold text-white sm:text-3xl">{count}</div>
            <div className="mt-1 truncate text-xs text-white/80">{formatToRupiah(pendingAmount)}</div>
          </div>
        </div>
      </CardContent>
    </Card>
