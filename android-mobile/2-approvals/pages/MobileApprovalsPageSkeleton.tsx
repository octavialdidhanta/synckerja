...
/** Selaras `ApprovalsDashboardCarousel` (`LOGICAL_SLIDE_COUNT = 4`). */
const CAROUSEL_DOT_COUNT = 4;
/** Selaras `ApprovalTable` mobile skeleton (11 kolom × 10 baris, viewport tetap). */
const APPROVALS_TABLE_COLUMNS = 11;
const APPROVALS_TABLE_BODY_ROWS = 10;

const MOBILE_APPROVALS_TABLE_VIEWPORT_CLASS =
  "h-[min(28rem,calc(100dvh-14rem))] max-h-[28rem] min-h-[11rem] shrink-0";

const TABLE_SCROLL_HIDE =
  "scrollbar-hide [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden";

/** Carousel loading — dipakai route shell + `ApprovalsDashboardCarousel`. */
export function MobileApprovalsCarouselSkeleton() {
  return (
...
/** Baris tbody — mirror `ApprovalTable` `skeletonRows` (mobile). */
export function MobileApprovalsTableBodyRows() {
  return (
    <>
      {Array.from({ length: APPROVALS_TABLE_BODY_ROWS }).map((_, rowIndex) => (
        <tr key={rowIndex} className="border-b border-border">
          {Array.from({ length: APPROVALS_TABLE_COLUMNS }).map((__, ci) => (
...
function MobileApprovalsTableCardSkeleton() {
  return (
      ...
      <CardContent className="flex min-h-0 min-w-0 flex-col p-0">
        <div className="min-w-0 flex-shrink-0 border-b bg-muted/50 px-1.5 py-1.5">
          <div className="flex w-full min-w-0 items-center gap-1">
            ...
            <Skeleton className="h-9 w-[7.25rem] shrink-0 rounded-md" aria-hidden />
        ...
        <div
          className={cn(
            ...
            TABLE_SCROLL_HIDE,
            MOBILE_APPROVALS_TABLE_VIEWPORT_CLASS,
          )}
        >
          <table className="min-w-[1300px] w-full border-collapse text-sm caption-bottom">
            <thead className="sticky top-0 z-10 border-b border-border bg-muted">
              <tr>
                {Array.from({ length: APPROVALS_TABLE_COLUMNS }).map((_, i) => (
...
export type MobileApprovalsPageSkeletonProps = {
  className?: string;
};

export function MobileApprovalsPageSkeleton({ className }: MobileApprovalsPageSkeletonProps) {
  return (
    <div className={cn("flex min-h-0 min-w-0 flex-1 flex-col gap-1", className)}>
      <div className="shrink-0">
        <MobileApprovalsCarouselSkeleton />
      </div>
      <div className="flex min-h-0 min-w-0 shrink-0 flex-col">
        <MobileApprovalsTableCardSkeleton />
      </div>
    </div>
  );
}

export type MobileApprovalsChromeSkeletonProps = {
  wrapperClassName?: string;
};

export function MobileApprovalsChromeSkeleton({ wrapperClassName }: MobileApprovalsChromeSkeletonProps) {
  const { t } = useAppTranslation();
  const aria = t("approvals.route.loadingAria", "Loading approvals");
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        <div className="scrollbar-hide flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden overflow-y-auto seamless-scroll [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          <div className="flex min-h-full min-w-0 flex-1 flex-col">
            ...
            <MobileApprovalsPageSkeleton className="mx-auto flex min-h-0 w-full max-w-md flex-1 flex-col content-padding-above-nav-default px-2 pt-2" />
          </div>
        </div>
      </div>
...
export function MobileApprovalsShellSkeleton() {
  return (
    <div className="flex min-h-screen w-full bg-background">
      <MobileApprovalsChromeSkeleton />
    </div>
  );
}

export function MobileApprovalsFullViewportOverlay() {
  return (
    <MobileApprovalsChromeSkeleton wrapperClassName="fixed inset-0 z-[200] min-h-[100dvh] bg-background" />
  );
}
