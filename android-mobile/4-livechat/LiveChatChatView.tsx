...
import { Sheet, SheetContent, SheetHeader, SheetTitle, SheetTrigger } from '@/mobile-app/components/ui/sheet';
...
import { ChatThread } from '@/5-3-whatsapp/components/inbox/ChatThread';
import { EmailChatThread } from '@/5-3-whatsapp/components/inbox/EmailChatThread';
    ...
    <SidebarProvider>
      <div className="fixed inset-x-0 z-0 flex flex-col bg-background" style={mainFixedStyle}>
        ...
        <div className="relative shrink-0">
          <Avatar className="h-10 w-10 rounded-full border-2 border-primary-foreground/25 bg-primary-foreground/15">
            <AvatarImage src={profileUrl ?? undefined} alt={displayName} className="object-cover" />
            <AvatarFallback className="rounded-full bg-primary-foreground/15 text-sm font-medium text-primary-foreground">
              {initials || <User className="h-5 w-5" />}
            </AvatarFallback>
          </Avatar>
          <span
            className={`absolute -right-0.5 -bottom-0.5 flex h-5 w-5 items-center justify-center rounded-full border-2 border-primary ${overlayColor} text-white`}
            aria-hidden
          >
        ...
        <div className="flex-1 min-w-0">
          <h2 className="truncate font-semibold text-primary-foreground">{displayName}</h2>
          {subText && <p className="truncate text-xs text-primary-foreground/85">{subText}</p>}
        </div>
        <Sheet open={quickActionOpen} onOpenChange={setQuickActionOpen}>
          <SheetTrigger asChild>
            <Button type="button" variant="ghost" size="icon" className="h-9 w-9 shrink-0 text-primary-foreground hover:bg-primary-foreground/15 hover:text-primary-foreground" aria-label={t('whatsappInbox.quickAction', 'Quick Action')}>
